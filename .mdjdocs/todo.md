...
- [x] Auditi mrtve flagove — ODLUKA: implementirane provere za sva tri. `feature_wishlist` (API 403 + UI base+sloj), `feature_compare` (UI-only — nema API povrsinu), `feature_multi_currency` (admin CRUD 403 + useCurrency skip). Testovi: `WishlistFeatureFlagTest`, `CurrencyFeatureFlagTest` (2026-07-14)
- [x] [SECURITY, otkriveno] `feature_loyalty` — checkout gate (422) + `loyalty/balance` i `store-credits/balance` rute pod `feature:` middleware + poeni/krediti stranice 404 guard (base+sloj). Testovi: `LoyaltyFeatureFlagTest` (2026-07-14)
