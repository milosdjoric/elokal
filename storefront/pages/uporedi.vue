...
useHead({ title: 'Poređenje proizvoda — eLokal' })
</script>
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full border-collapse">
        ...
        <tr class="border-b border-gray-100">
          ...
          <td v-for="product in store.items" :key="product.id" class="py-3 px-4">
            <span class="text-sm font-bold text-primary-600">{{ formatPrice(product.effective_price) }}</span>
            <span v-if="product.is_on_sale" class="text-xs text-gray-400 line-through ml-2">{{ formatPrice(product.price) }}</span>
          </td>
        </tr>
