      ...
      <div class="max-w-7xl mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-2">
          ...
          <div v-if="prevProduct || nextProduct" class="flex items-center gap-2 text-sm">
            <NuxtLink v-if="prevProduct" :to="`/proizvodi/${prevProduct.slug}`" class="text-gray-400 hover:text-gray-700" :title="prevProduct.name">
              ← Prethodni
            </NuxtLink>
            <span v-if="prevProduct && nextProduct" class="text-gray-300">|</span>
            <NuxtLink v-if="nextProduct" :to="`/proizvodi/${nextProduct.slug}`" class="text-gray-400 hover:text-gray-700" :title="nextProduct.name">
              Sledeći →
            </NuxtLink>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div>
            ...
            <!-- Social proof -->
            <p v-if="viewerCount > 1" class="text-xs text-orange-600 font-medium mb-1">
              <svg class="w-3.5 h-3.5 inline-block mr-0.5 -mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z" /><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" /></svg>
        ...
        <section v-if="product.up_sell_products?.length" class="mb-12">
          <ProductCarousel title="Možda vas zanima i..." :products="product.up_sell_products" @quick-view="openQuickView" />
        </section>

        <!-- Related products -->
        <section v-if="related.length" class="mb-12">
          <ProductCarousel title="Slični proizvodi" :products="related" @quick-view="openQuickView" />
        </section>

        <!-- Recently viewed -->
        <section v-if="recentlyViewed.length > 0" class="mb-12">
          <ProductCarousel title="Nedavno pregledano" :products="recentlyViewed" @quick-view="openQuickView" />
        </section>
      </div>
