...
async function handleSubmit() {
  ...
  try {
    ...
    clearCart()
    await router.push(`/porudzbina/success/${data.data.order_number}`)
  }
  catch (e: unknown) {
...
useHead({ title: 'Kasa — sloj kolektiv' })
</script>

<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    <LayoutBreadcrumbs :items="[{ label: 'Korpa', to: '/korpa' }, { label: 'Kasa' }]" />
          ...
          <div class="border border-gray-200 p-6 sticky top-20">
            ...
            <div class="mt-6 pt-4 border-t border-gray-100 space-y-3">
              <div class="flex items-center gap-2 text-xs text-gray-500">
                <Icon name="lucide:lock" class="w-4 h-4 text-success flex-shrink-0" />
                Sigurna kupovina — vaši podaci su zaštićeni
              </div>
              <div class="flex items-center gap-2 text-xs text-gray-500">
                <Icon name="lucide:check" class="w-4 h-4 text-success flex-shrink-0" />
                Besplatna dostava na sve narudžbine
              </div>
              <div class="flex items-center gap-2 text-xs text-gray-500">
                <Icon name="lucide:rotate-ccw" class="w-4 h-4 text-success flex-shrink-0" />
