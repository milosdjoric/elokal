...
<template>
  <div class="space-y-6">
    ...
    <div class="border border-gray-200 p-4">
      <div v-if="isFreeShipping(cartStore.total)" class="flex items-center gap-2 text-sm text-success font-medium">
        <Icon name="lucide:check-circle-2" class="w-5 h-5" />
        Ostvarili ste besplatnu dostavu!
      </div>
      <div v-else>
        <p class="text-sm text-gray-600 mb-2">
          Još <span class="font-semibold text-primary-600 tabular-nums">{{ formatPrice(remainingForFree(cartStore.total)) }}</span> do besplatne dostave
        </p>
    ...
    <div class="border border-gray-200 p-6 space-y-3">
      ...
      <div class="flex justify-between text-sm">
        ...
        <span v-else-if="estimatedCost(cartStore.total) !== null" class="font-medium tabular-nums">
          {{ formatPrice(estimatedCost(cartStore.total)!) }}
        </span>
        <span v-else class="text-gray-400">Izračunava se u sledećem koraku</span>
      </div>
      <div class="flex justify-between text-sm">
        <span class="text-gray-600">Porez (PDV {{ taxRate }}%)</span>
        <span v-if="taxAmount > 0" class="font-medium tabular-nums">~{{ formatPrice(taxAmount) }}</span>
        <span v-else class="text-gray-400">—</span>
      </div>
      <hr class="my-2" />
      <div class="flex justify-between text-lg font-bold">
        <span>Ukupno</span>
        <span class="tabular-nums">{{ formatPrice(cartStore.total + taxAmount + (isFreeShipping(cartStore.total) ? 0 : (estimatedCost(cartStore.total) ?? 0))) }}</span>
      </div>
      ...
      <div class="mt-3 pt-3 border-t border-gray-100">
        <div v-if="appliedCoupon" class="flex items-center justify-between bg-green-50 border border-green-200 px-3 py-2 mb-3">
          <div>
            ...
            <p class="text-xs text-green-600 tabular-nums">−{{ formatPrice(appliedCoupon.discount) }}</p>
          </div>
    ...
    <div v-if="crossSellProducts.length > 0" class="border border-gray-200 p-6">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          />
          <div v-else class="w-12 h-12 bg-placeholder rounded flex-shrink-0" />
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-gray-800 truncate">{{ product.name }}</p>
            <p class="text-sm text-primary-600 tabular-nums">{{ formatPrice(product.effective_price) }}</p>
          </div>
        </NuxtLink>
      </div>
    </div>
