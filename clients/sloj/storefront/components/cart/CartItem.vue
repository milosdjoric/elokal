...
const lineTotal = computed(() => formatPrice(parseFloat(props.item.product.effective_price) * props.item.quantity))
</script>
...
<template>
  <div class="flex gap-3">
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="text-xs text-gray-500 mt-0.5 tabular-nums">{{ formatPrice(item.product.effective_price) }}</p>
      <div class="flex items-center justify-between mt-2">
        <UiMoleculesQuantitySelector v-model="item.quantity" @update:model-value="updateQuantity(item.product.id, $event)" />
        <div class="text-right">
          <p class="text-sm font-semibold tabular-nums">{{ lineTotal }}</p>
